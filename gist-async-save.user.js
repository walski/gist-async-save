...
if (window.location.href.match(/\/edit$/)) {
  ...
  execute(function() {
    ...
    var Gist = function() {
      gist = this;
      var saveButton = jQuery('button:contains(Save Gist)');
      var form       = saveButton.parents('form');

      // Build the new button and spinner
      var saveAndStayButton = saveButton.clone();
      saveButton.css('margin', '5px 10px');
      saveAndStayButton.html('<span></span>');
      ...
      var enableSaveAndStayButton = function() {
        saveAndStayButton.removeAttr('disabled');
        saveAndStayButton.find('img').hide();
        saveAndStayButton.find('span').text('Save Gist and stay')
      }

      var disableSaveAndStayButton = function() {
        saveAndStayButton.find('span').text('Saving…')
        saveAndStayButton.find('img').show();
        saveAndStayButton.attr('disabled', 'disabled');
      }

      // Visually signal success or failure of the asynchronous save
      var highlightEditors = function(options) {
        jQuery('textarea.file-contents').effect('highlight', options);
      }

      // Regoister the asynchronous save method for clicks on the button
      saveAndStayButton.click(function(e) {
        e.preventDefault();
        gist.sendAsynchronously();
      })

      // Make keymaster work in the Gist edit textareas and input fields
      key.filter = function filter(event){
        return true;
      }

      // Hooks up the keyboard shortcuts to the asynchronous save method
      key('⌘+s, ctrl+s', function(e, handler){
        e.preventDefault();
        gist.sendAsynchronously();
      });

      // Insert the button in the right state before the normal save button
      enableSaveAndStayButton();
      saveButton.before(saveAndStayButton);
      // Freeze the buttons width
      saveAndStayButton.css('width', saveAndStayButton.width());

      // Save the Gist via AJAX
      gist.sendAsynchronously = function() {
        var url  = form.attr('action')
        var data = form.serialize();

        disableSaveAndStayButton();
        jQuery.post(url, data).
          ...
          error(function (jqXHR, status) {
            saveAndStayButton.find('img').hide();
            saveAndStayButton.find('span').text('An error occured.');
            if (console && console.log && console.group && console.groupEnd) {
              highlightEditors({color: '#900', duration: 500})
              console.group("Error while saving this Gist asynchronously:");
                console.log(status);
                console.log(jqXHR);
              console.groupEnd();
            }
            alert('Gist could not be saved. Please use the normal save button instead.');
          });
      };
    };

    // Run the whole userscript
    new Gist();
  });
}